...
@callback(
    ...
    Output('admin-tabs', 'active_tab', allow_duplicate=True),
...
def submit_new_post(n_clicks, title, content, tier, category, duration,
    ...
    if not title or not content:
        return dbc.Alert("Title and content are required", color="danger", dismissable=True), no_update

    if not user_session or user_session.get('access_tier', 0) < 4:
        return dbc.Alert("Admin access required", color="danger", dismissable=True), no_update

    # Sanitize inputs
    import html as html_module
    title = html_module.escape(title.strip())
    content = html_module.escape(content.strip())
    ...
    try:
        ...
        if post_id:
            return (
                ...
                'posts-management'  # Switch to posts tab to see the new post
            )
        else:
            return dbc.Alert("Error creating post", color="danger", dismissable=True), no_update

    except Exception as e:
        return dbc.Alert(f"Error: {str(e)}", color="danger", dismissable=True), no_update
